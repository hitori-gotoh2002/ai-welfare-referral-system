...
function serviceScore(service, needs) {
  ...
  return overlap * 4 + urgent * 2 + region;
}
...
function ensureCoverage(items, needs) {
  ...
  needs.slice(0, 4).forEach((need) => {
    if (!domains.has(need)) {
      const found = services.find((service) => service.domains.includes(need) && !selected.some((item) => item.id === service.id));
      if (found) selected.push(found);
    }
  });
  if (new Set(selected.flatMap((item) => item.domains)).size < 2) {
    const add = services.find((service) => !selected.some((item) => item.id === service.id));
...
function copyReport() {
  ...
  const lines = report
    ? [
        ...
        "",
        "추천 서비스",
        ...report.services.map((service, index) => `${index + 1}. ${service.name} - ${service.summary}`),
        "",
        "확인 조건",
        ...report.conditions.map((condition) => `- ${condition.service}: ${condition.text}`),
        "",
        "준비 서류",
        ...
        <article class="report-preview">
          ...
          <div class="report-content">
            ...
            <section class="report-block">
              ...
              <ol>
                ${reportServices
                  .map(
                    (service) =>
                      `<li><strong>${escapeHtml(service.name)}</strong> · ${escapeHtml(service.summary)} <span class="tiny">(${escapeHtml(service.source)}, ${escapeHtml(service.updated)})</span></li>`
                  )
                  .join("")}
              </ol>
            </section>
            <section class="report-block">
              <h3>확인 조건</h3>
              <ul>
                ${reportConditions.map((condition) => `<li>${escapeHtml(condition.service)}: ${escapeHtml(condition.text)}</li>`).join("")}
              </ul>
            </section>
            <section class="report-block">
              <h3>준비 서류</h3>
              <div class="pill-row">${reportDocs.map((doc) => pill(doc)).join("")}</div>
            </section>
            <section class="report-block">
              <h3>연결 순서</h3>
              <ol>
                ${reportSteps.map((step) => `<li>${escapeHtml(step)}</li>`).join("")}
              </ol>
            </section>
            <section class="report-block">
              <h3>문의처</h3>
