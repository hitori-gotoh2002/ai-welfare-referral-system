...
def xml_all_text(element: ElementTree.Element, name: str) -> list[str]:
    return [clean_text(item.text or "") for item in element.findall(name) if clean_text(item.text or "")]


def xml_items(root: ElementTree.Element) -> list[ElementTree.Element]:
    candidates = []
    for element in root.iter():
        if xml_text(element, "servId", "wlfareInfoId") and xml_text(element, "servNm", "wlfareInfoNm", "bizNm"):
...
def fetch_public_welfare_detail(service: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        detail = {
            **service,
            "name": xml_text(root, "servNm") or service.get("name", ""),
            "summary": xml_text(root, "wlfareInfoOutlCn", "servDgst") or service.get("summary", ""),
            "eligibility": xml_text(root, "tgtrDtlCn", "sprtTrgtCn") or service.get("eligibility", ""),
            "selectionCriteria": xml_text(root, "slctCritCn"),
            "support": xml_text(root, "alwServCn") or service.get("support", ""),
            "process": xml_text(root, "aplyMtdCn") or " → ".join(application_steps) or service.get("process", ""),
            "docs": unique([*service.get("docs", []), *forms]) or service.get("docs", []),
            "contact": xml_text(root, "rprsCtadr") or (contacts[0] if contacts else service.get("contact", "")),
            "url": service.get("url") or (homepages[0] if homepages else "https://www.bokjiro.go.kr"),
            "updated": xml_text(root, "crtrYr", "lastModYmd") or service.get("updated", ""),
...
def service_score(service: dict[str, Any], needs: list[str], case: dict[str, Any]) -> int:
    ...
    return overlap * 4 + urgent * 2 + region_match
...
def filter_local_services(query: dict[str, list[str]], catalog: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    ...
    needs = [item for item in query.get("needs", [""])[0].split(",") if item]
    case = {"region": query.get("region", [""])[0] or ""}
...
def ensure_coverage(
    items: list[dict[str, Any]], needs: list[str], catalog: list[dict[str, Any]] | None = None
) -> list[dict[str, Any]]:
    selected = list(items)
    source_catalog = catalog or SERVICES
    ...
    for need in needs[:4]:
        if need not in domains:
            found = next(
                (service for service in source_catalog if need in service["domains"] and service not in selected),
                None,
            )
            if found:
    ...
    if len({domain for service in selected for domain in service["domains"]}) < 2:
        fallback = next((service for service in source_catalog if service not in selected), None)
        if fallback:
...
def generate_packages(
    ...
    source_catalog = catalog or SERVICES
    ranked = sorted(
        [service for service in source_catalog if service_score(service, needs, case) > 0],
        key=lambda service: service_score(service, needs, case),
        reverse=True,
    )
    ...
    top_a = ensure_coverage(mixed[:4], needs, source_catalog)
